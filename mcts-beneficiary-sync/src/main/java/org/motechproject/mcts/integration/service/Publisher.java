...
public class Publisher {
     ...
     * public void handleEvent(MotechEvent motechEvent) throws
     * BeneficiaryException{ String url =
     * (String)motechEvent.getParameters().get
     * (MCTSEventConstants.PARAM_PUBLISHER_URL); publish(url); }
     */
    /**
    ...
    public void publish(String url) throws BeneficiaryException {
        ...
        if (response.getStatusCode().value() / 100 == 2)
            ...
            LOGGER.error(error);
            throw new BeneficiaryException(ApplicationErrors.CONNECTION_ERROR,
                    error);
        }
    }
