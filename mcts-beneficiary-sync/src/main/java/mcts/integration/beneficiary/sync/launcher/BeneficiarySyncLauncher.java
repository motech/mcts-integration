...
public class BeneficiarySyncLauncher {
    ...
    private static BeneficiarySyncServiceFactory getBeneficiarySyncServiceFactory() {
        ...
        context.refresh();
        return (BeneficiarySyncServiceFactory) context.getBean("beneficiarySyncServiceFactory");
    }
    ...
    private static void validateArguments(String[] syncArgs) {
        if (syncArgs.length != 3)
            throw new IllegalArgumentException("Invalid arguments. Expected 3 arguments(in order): SyncType, StartTime and EndTime");
    ...
    private static void validateSyncType(String syncType) {
        if (!SyncType.isValid(syncType))
            throw new IllegalArgumentException(String.format("Invalid sync type. Sync type should be %s or %s", SyncType.GET.getDescription(), SyncType.UPDATE.getDescription()));
    }
