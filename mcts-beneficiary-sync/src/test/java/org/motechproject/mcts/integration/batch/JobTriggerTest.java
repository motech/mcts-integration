...
public class JobTriggerTest {
	...
	@InjectMocks MctsJobTrigger jobTrigger = new MctsJobTrigger(restTemplate, batchServiceUrlGenerator,httpAgentServiceOsgi);
	...
	public void triggerJob_success() {
		...
		verify(httpAgentServiceOsgi).executeSync((String) any(), anyObject(), Matchers.any(Method.class));
		verify(httpAgentServiceOsgi, times(1)).executeSync((String) any(), anyObject(), Matchers.any(Method.class));
	}
