...
public class PropertyReaderTest {
    ...
    @InjectMocks
    private PropertyReader propertyReader = Mockito.spy(new PropertyReader(
            properties));

    @Test
    public void shouldGetSyncUrl() throws BeneficiaryException {
        when(properties.getProperty("mcts.base.url")).thenReturn("baseUrl");
        when(properties.getProperty("beneficiary.sync.update.request.url"))
        ...
        when(
                ...
                .thenReturn("username");
        when(properties.getProperty("mcts.authentication.username"))
                .thenReturn("myUser");
        when(
                properties
                        .getProperty("beneficiary.sync.update.request.authentication.password.key"))
                .thenReturn("password");
        when(properties.getProperty("mcts.authentication.password"))
        ...
        assertEquals(
                "baseUrl/requestUrl?username=myUser&password=myPassword&operation=update",
                syncUrl);
    }
    ...
    public void shouldGetDefaultBeneficiaryListQueryParams()
        ...
        when(properties.getProperty("mcts.authentication.password"))
                .thenReturn("myPassword");
        doReturn("myPassword").when(propertyReader).getPassword();
        MultiValueMap<String, String> defaultQueryParams = propertyReader
